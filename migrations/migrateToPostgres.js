      ...
      if (tenant.shopify) {
        ...
        if (tenant.shopify.schedules) {
          await prisma.scheduleTenant.create({
            data: {
              tenantId: tenantInfo.id,
              nebimProductInventoryInterval: tenant.shopify.schedules.nebim?.product?.inventory?.interval || '0 * * * *',
              ...
              nebimProductDetailsInterval: tenant.shopify.schedules.nebim?.product?.details?.interval || '0 0 * * *',
